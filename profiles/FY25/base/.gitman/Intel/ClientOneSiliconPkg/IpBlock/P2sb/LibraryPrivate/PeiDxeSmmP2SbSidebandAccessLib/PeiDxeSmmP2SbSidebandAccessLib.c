  ...
  )
{
  ...
  *Response = SBI_INVALID_RESPONSE;
  ...
  if (IsSimicsEnvironment ()) {
    //
    // Simics seems to have a problem with FBE set to
    // other values then 0xF. For now override the value to
    // 0xF to work around this issue.
    //
    Fbe = 0xF;
  }
  PciSegmentWrite16 (
  ...
  if (Timeout == 0) {
    ...
    *Response = (UINT8) ((SbiStat & B_P2SB_CFG_SBISTAT_RESPONSE) >> N_P2SB_CFG_SBISTAT_RESPONSE);
    if (*Response == SBI_SUCCESSFUL) {
      ...
      DEBUG ((DEBUG_ERROR, "P2SB: MSG failed PID:%X, Resp:%d\n", Pid, *Response));
      if (*Response == SBI_POWERDOWN) {
...
VOID
P2SbByteAccessAlignToDword (
  IN UINT32   Offset,
  OUT UINT32  *AlignedOffset,
  OUT UINT16  *ByteEnable
  )
{
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    ...
    case P2SbMsgAccess:
      P2SbByteAccessAlignToDword (Offset, &AlignedOffset, &ByteEnable);
      Status = P2SbSbiExecutionEx (
                Private->P2SbCtrl->PciCfgBaseAddr,
                Private->P2SbPid,
                AlignedOffset,
  ...
  )
{
  ...
  switch (Private->AccessMethod) {
    ...
    case P2SbMsgAccess:
      P2SbWordAccessAlignToDword (Offset, &AlignedOffset, &ByteEnable);
      Status = P2SbSbiExecutionEx (
                Private->P2SbCtrl->PciCfgBaseAddr,
                Private->P2SbPid,
                AlignedOffset,
